...
SERVER_URL = os.environ.get("FLUX_MCP_SERVER_URL")
SERVER_TOKEN = os.environ.get("FLUX_MCP_TOKEN")

# Both of these are required
if not SERVER_URL or not SERVER_TOKEN:
    raise ValueError("You must export the FLUX_MCP_SERVER_URL and FLUX_MCP_TOKEN")

# Assume we are dumb.
if not SERVER_URL.endswith("mcp"):
    print("Warning: the server url likely needs to end with mcp. I will add it for you.")
...
async def run_test():
    ...
    try:
        async with Client(
            transport=StreamableHttpTransport(SERVER_URL, headers={"x-token": SERVER_TOKEN})
        ) as client:
